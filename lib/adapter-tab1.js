...
var adapterTab1 =  function (_tab1RealJson) {
    ...
    adapter.execute = function() {
      ...
      // console.log({nodeOnlySourceTarget});
      var nodeInTheMidlle = _tab1RealJson.graph.nodes.filter(function (n) {
        //console.log('[--->]'+ n.name);
        for (idx0 in nodeSourceTarget) {
          ...
          if (nodeSourceTarget[idx0].subnode_input !== undefined){
            for (idx1 in nodeSourceTarget[idx0].subnode_input){
              // console.log('[**]' + nodeSourceTarget[idx0].subnode_input[idx1].name);
      ...
      adapter.tab1Adapted.graph.nodes = nodeOnlySource.concat(nodeInTheMidlle).concat(nodeOnlyTarget);
