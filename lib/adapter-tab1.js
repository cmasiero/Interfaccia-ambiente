...
var adapterTab1 = function (_tab1RealJson) {
  ...
  adapter.execute = function () {
    ...
    var isInDirection = function (n, direction) {
      for (l in _tab1RealJson.graph.links) {
    ...
    for (idx in _tab1RealJson.graph.links) {
      ...
      if (removeOnTheLeft(l)) {
        continue;
      }

      if (removeOnTheRight(l)) {
        continue;
      }

      if (changeBySubnodeSource(l)) {
        continue;
      }

      if (changeBySubnodeTarget(l)) {
        continue;
      }
   ...
   */
    function changeBySubnodeTarget(l) {
      var result = false;
    ...
    function changeBySubnodeSource(l) {
      var result = false;
